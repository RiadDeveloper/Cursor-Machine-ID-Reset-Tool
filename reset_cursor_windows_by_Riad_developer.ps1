...
function Show-LoadingAnimation {
    $loadingChars = "⣾⣽⣻⢿⡿⣟⣯⣷"
    $loadingMsg = "Initializing Cursor Reset Process"
    $counter = 0
    $totalDuration = 3  # 3 seconds
    $startTime = Get-Date
    
    while (((Get-Date) - $startTime).TotalSeconds -lt $totalDuration) {
        $currentChar = $loadingChars[$counter % $loadingChars.Length]
        Write-Host "`r$loadingMsg $currentChar" -NoNewline -ForegroundColor Cyan
        Start-Sleep -Milliseconds 100
        $counter++
    }
    Write-Host "`r$loadingMsg Complete!     " -ForegroundColor Green
    Start-Sleep -Seconds 1
    Clear-Host
}

Write-Host "`n`n============================================================" -ForegroundColor Magenta
Write-Host "                   CURSOR RESET TOOL                         " -ForegroundColor Cyan
Write-Host "                Created by: Riad developer                   " -ForegroundColor Yellow
